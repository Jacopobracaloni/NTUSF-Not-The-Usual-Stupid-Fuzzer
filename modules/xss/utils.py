...
def getVar(name):
    return modules.xss.config.globalVariables[name]


def updateVar(name, data, mode=None):
    if mode:
        if mode == "append":
            modules.xss.config.globalVariables[name].append(data)
        elif mode == "add":
            modules.xss.config.globalVariables[name].add(data)
    else:
        modules.xss.config.globalVariables[name] = data
