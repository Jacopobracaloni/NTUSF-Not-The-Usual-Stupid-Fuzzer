...
if __name__ == "__main__":
    try:
        ...
        parser.add_argument(
            ...
            nargs="*",
            default=[
                "XSSscanner", "SQLscanner", "CSRFscanner", "Traversalscanner",
                "OpenRedirectScanner", "SSRFscanner", "SSTIscanner",
                "CMDInjectionscanner",
            ],
        ...
        if testing_connection(args.url, proxies) == True:
            ...
            if cookies is None:
                cookies = {}
                logger.warning("No cookies loaded. Proceeding without cookies.\n")
            ...
            merged_df = (
                xss_df
                .merge(sql_df[["URL", "SQLi"]], on="URL", how="left")
                .merge(csrf_df[["URL", "CSRF"]], on="URL", how="left")
                .merge(path_traversal_df[["URL", "Path_Traversal"]], on="URL", how="left")
            )

            # Optional new modules — merge only if the CSV exists
            _new_modules = {
                "Open_Redirect": DF + f"{base_url}_open_redirect.csv",
                "SSRF": DF + f"{base_url}_ssrf.csv",
                "SSTI": DF + f"{base_url}_ssti.csv",
                "CMD_Injection": DF + f"{base_url}_cmd_injection.csv",
            }
            import os as _os
            for col, path in _new_modules.items():
                if _os.path.exists(path):
                    _tmp = pd.read_csv(path)
                    if col in _tmp.columns:
                        merged_df = merged_df.merge(
                            _tmp[["URL", col]], on="URL", how="left"
                        )
            ...
            report_path = f"{sys.path[0]}/results/{base_url}/report.html"
            generate_report(base_url, merged_df, report_path)
