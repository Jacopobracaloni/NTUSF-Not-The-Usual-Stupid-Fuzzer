...
_STATIC_EXTENSIONS = frozenset([
    ...
    ".bmp", ".tiff", ".avif", ".pdf", ".zip", ".tar", ".gz",
])

_BORING_PATHS = frozenset([
    "robots.txt", "sitemap.xml", "favicon.ico", "favicon.png",
    "humans.txt", "manifest.json", "browserconfig.xml",
    "crossdomain.xml", "apple-touch-icon.png", "service-worker.js",
    "sw.js", ".htaccess", ".htpasswd",
])

_API_KEYWORDS = frozenset([
    "api", "rest", "graphql", "gql", "v1", "v2", "v3", "rpc",
])

_AUTH_KEYWORDS = frozenset([
    "login", "logout", "signin", "signup", "register", "auth",
    "password", "token", "forgot", "reset", "verify", "oauth",
    "session", "credential", "whoami", "me", "profile",
])

_ADMIN_KEYWORDS = frozenset([
    "admin", "management", "dashboard", "console", "superuser",
    "backoffice", "panel", "metrics", "actuator", "monitor",
])

_REDIRECT_PARAMS = frozenset([
    "redirect", "next", "return", "url", "goto", "target", "dest",
    "returnurl", "back", "continue", "forward", "location",
])

_SENSITIVE_PARAMS = frozenset([
    "password", "passwd", "pass", "pwd", "token", "secret", "key",
    "api_key", "apikey", "auth", "authorization", "username", "user",
    "email", "userid", "user_id", "id", "admin", "file", "path",
    "src", "callback", "session", "csrf", "nonce", "redirect",
    "next", "return", "url", "goto", "target", "dest",
])
...
class URLClassifier:
    def __init__(self, base_url, threshold=0.3, n_estimators=1000):
        ...
        self._use_v2 = False   # set to True when v2 dataset is detected
    ...
    def _extended_features(self, urls, methods=None, post_params_list=None):
        """Extract structural endpoint features beyond TF-IDF and suspicious tokens."""
        n = len(urls)
        if methods is None:
            methods = ["GET"] * n
        if post_params_list is None:
            post_params_list = ["{}"] * n

        has_static_ext  = np.zeros(n)
        has_numeric_id  = np.zeros(n)
        is_api_path     = np.zeros(n)
        method_is_post  = np.zeros(n)
        has_post_params = np.zeros(n)
        num_post_params = np.zeros(n)
        has_sensitive_param = np.zeros(n)
        path_depth      = np.zeros(n)
        has_redirect_param = np.zeros(n)
        has_auth_keyword   = np.zeros(n)
        has_admin_keyword  = np.zeros(n)
        has_boring_path    = np.zeros(n)

        for i, (url, method, pp) in enumerate(zip(urls, methods, post_params_list)):
            ...
            # boring well-known path
            basename = os.path.basename(path).lstrip(".")
            if basename in _BORING_PATHS or path.lstrip("/") in _BORING_PATHS:
                has_boring_path[i] = 1
            ...
            if parts & _API_KEYWORDS:
                is_api_path[i] = 1

            # auth / admin keywords
            if parts & _AUTH_KEYWORDS or any(k in path for k in _AUTH_KEYWORDS):
                has_auth_keyword[i] = 1
            if parts & _ADMIN_KEYWORDS or any(k in path for k in _ADMIN_KEYWORDS):
                has_admin_keyword[i] = 1
            ...
            # POST body params
            try:
                body = json.loads(pp) if pp and pp != "{}" else {}
            except Exception:
                body = {}
            if body:
                has_post_params[i] = 1
                num_post_params[i] = len(body)
                if any(k.lower() in _SENSITIVE_PARAMS for k in body):
                    has_sensitive_param[i] = 1

            # Sensitive GET params
            if not has_sensitive_param[i]:
                get_params = set(k.lower() for k in parse_qs(query).keys())
                if get_params & _SENSITIVE_PARAMS:
                    has_sensitive_param[i] = 1
                if get_params & _REDIRECT_PARAMS:
                    has_redirect_param[i] = 1

            # Path depth
            path_depth[i] = len([p for p in path.split("/") if p])
    ...
    def domain_features(self, urls, methods=None, post_params_list=None):
        ...
        # Extended structural features (v2 dataset)
        if self._use_v2:
            ext_feats = self._extended_features(urls, methods, post_params_list)
            features = np.hstack([features, ext_feats])
    ...
    def load_data(self):
        try:
            ...
            # Prefer the purpose-built v2 dataset; fall back to CSIC 2010
            v2_path = sys.path[0] + "/ML/Data_Files/Training_data_v2.csv"
            v1_path = sys.path[0] + "/ML/Data_Files/Training_data.csv"
            if os.path.exists(v2_path):
                self.logger.info("Using purpose-built v2 training dataset\n")
                self.training_data = pd.read_csv(v2_path)
                self._use_v2 = True
            else:
                self.logger.info("v2 dataset not found — falling back to CSIC 2010\n")
                self.training_data = pd.read_csv(v1_path)
                self._use_v2 = False
    ...
    def preprocess_data(self):
        try:
            ...
            # Pass method / POST params when available (v2 dataset + crawled data)
            def _col(df, col, n, default=""):
                if col in df.columns:
                    return df[col].fillna(default).tolist()
                return [default] * n

            train_methods     = _col(self.training_data, "Method",      len(train_urls), "GET")
            train_post_params = _col(self.training_data, "POST Params", len(train_urls), "{}")
            test_methods      = _col(self.test_data,     "Method",      len(test_urls),  "GET")
            test_post_params  = _col(self.test_data,     "POST Params", len(test_urls),  "{}")

            X_train_domain = self.domain_features(train_urls, train_methods, train_post_params)
            X_test_domain  = self.domain_features(test_urls,  test_methods,  test_post_params)
